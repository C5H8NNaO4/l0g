...
class Color extends MapFormatter {
  constructor(fn, options = {}) {
    const {
      strFormatMap = Color.strFormatMap,
      formatMap = Color.formatMap,
      optionsMap = Color.optionsMap,
      colors = Color.colors
    } = options;
    super(fn, { ...options,
      strFormatMap,
...
Color.formatMap = new Map([[Color.isString, [(value, formatter) => !formatter.colors.type.string ? value : chalk.keyword(formatter.colors.type.string)(value)]], [oftype('number'), chalk.keyword('cyan')], [Color.isArray, [Color.colorByType('array')]], [Color.isError, (v, formatter) => Color.colorByType('error')(v.message, formatter)], [Color.isObject, [Color.colorByType('object')]], [/warning/i, (m, r) => m.replace(r, (val, formatter) => {
  const [bg, clr] = formatter.colors.highlight.split('.');
  return chalk.bgKeyword(bg).keyword(clr)(val);
})]]);
Color.strFormatMap = new Map([// [() => true, v => v]
]); //TODO Refactor using functions!
// () => test(), () => frmt();

Color.optionsMap = new Map([[key('ts'), function (v, formatter) {
