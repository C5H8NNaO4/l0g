const {
  getLine,
  isVisible
} = require('./util'); // const {Transport, ConsoleTransport} = require('./transports')
...
class Logger {
  ...
  constructor(level = Logger.defaults.level, options = {}) {
    const {
      transports,
      gather = null,
      levels = Logger.defaults.options.levels,
      features = [],
      extra = {}
    } = options;
    /**
    ...
    for (const transport of this.transports) {
      ...
      for (const feature of transport.features) feature.register.call(this, Logger);
  ...
  log(message, ...args) {
    ...
    for (const feature of this.features) {// options = feature.run.call(this, options);
    }
    ...
    if (options.scope && Logger.scope instanceof RegExp && !Logger.scope.test(options.scope)) return;
  ...
  scope(key) {
    ...
    if (this.scopes.has(nextScope)) return this.scopes.get(nextScope);
    const scoped = new Logger(this.level, { ...this.options,
      extra: { ...this.extra,
        scope: nextScope
      }
    });
...
Logger.prototype[Symbol.toStringTag] = 'Hi'; // Logger.prototype.transports = [new ConsoleTransport];

Logger.prototype.gather = {
  scope: () => '*',
  ts: () => +new Date() // loc: () => getLine(3),
