...
class Transport {
  ...
  transform(options) {
    ...
    if (this.formatter instanceof Formatter) {
      options.message = Formatter.format(this.formatter, {
        transport: this,
        ...options
      });
    }

    if (Array.isArray(options.tag)) options.tag = this.formatter.transformTagArgs(options, ...options.tag); // if (this.transform)
  ...
  send(options) {
    ...
    if (Logger.filter && Logger.filter instanceof RegExp && !Logger.filter.test(options.message)) return;
