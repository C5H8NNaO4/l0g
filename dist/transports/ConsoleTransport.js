...
function bind(that, obj) {
  ...
  for (const key in obj) if (typeof obj[key] === 'function') ret[key] = obj[key].bind(that);
...
class ConsoleTransportFeature extends TransportFeature {// static supports = [ConsoleTransport];
}
...
class ConsoleTransport extends Transport {
  ...
  log(options) {
    ...
    const args = ConsoleTransport.getOptionalArgs(options);
    if (logFn) _console[logFn](...args);
  }

}

ConsoleTransport.logFns = {
  warning: "warn",
  error: "error",
  debug: "debug",
  info: "info"
...
ConsoleTransport.surpress = () => {
  for (const key in console) console[key] = supress(key);
...
ConsoleTransport.unsurpressed = fn => {
  const {
    supress
  } = ConsoleTransport;
...
ConsoleTransport.getOptionalArgs = options => {
  const {
    console = {},
    message
  } = options;
